...
CREATE OR ALTER FUNCTION Sumatoria(@A as int =1, @B as int=2, @C as int=3)
    returns int
as
begin
    return @A + @B + @C
...
create or alter function CalcularTotal(@preciouni as money,@cantidad as int,@desc as float)
    returns money
as begin
    return @preciouni*@cantidad*(1-@desc)
end

select OrderID,dbo.CalcularTotal(UnitPrice,Quantity,Discount) from [Order Details]
