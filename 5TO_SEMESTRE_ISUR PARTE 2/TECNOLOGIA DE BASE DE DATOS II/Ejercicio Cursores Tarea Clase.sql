-- enviar correo por mes de las ventas de los clientes

select distinct FirstName + ' ' + LastName                                                           Empleado,
...
with cte1 as (select distinct E.EmployeeID,
...
select e.FirstName + ' ' + e.LastName, sum(o.Freight + os.Subtotal), OrderDate
from Employees E
...
from (select top 5 e.FirstName + ' ' + e.LastName Nombres, sum(o.Freight + os.Subtotal) Monto, OrderDate
      ...
      having year(OrderDate) = 1997
         and MONTH(OrderDate) = 5
         and e.FirstName + ' ' + e.LastName = 'Nancy Davolio') tc1

print @texto2

select top 5 e.FirstName + ' ' + e.LastName, sum(o.Freight + os.Subtotal), OrderDate
from Employees E
         inner join Orders O on E.EmployeeID = O.EmployeeID
         inner join [Order Subtotals] OS on O.OrderID = OS.OrderID
group by e.FirstName + ' ' + e.LastName, OrderDate
having year(OrderDate) = 1996
   ...
   and e.FirstName + ' ' + e.LastName = 'Nancy Davolio'
order by 2 desc
...
-- Comando para servidor de correo de prueba maildev --web 1081 --smtp 1026
