...
select c.CompanyName                                                                     Cliente,
       convert(money, sum((od.Quantity * od.UnitPrice) * (1 - od.Discount)) + o.Freight) MontoComprado
from Orders o
...
select distinct CompanyName,
                ProductName,
                convert(money, sum(MontoProducto) over ( partition by CompanyName,ProductName)) MontoFinal
from (select c.CompanyName,
      ...
      from Orders O
               inner join [Order Details] OD on O.OrderID = OD.OrderID
               inner join Products P on P.ProductID = OD.ProductID
               inner join Customers C on C.CustomerID = O.CustomerID) MontosProductoCliente
order by 1 asc, 3 desc

select row_number() over (partition by CompanyName order by CompanyName asc,MontoFinal desc) ID,
       CompanyName,
       ProductName,
       MontoFinal
from (select distinct CompanyName,
      ...
      from (select c.CompanyName,
            ...
            from Orders O
                     inner join [Order Details] OD on O.OrderID = OD.OrderID
                     inner join Products P on P.ProductID = OD.ProductID
                     inner join Customers C on C.CustomerID = O.CustomerID) MontosProductoCliente) TVA1
